...
to absorb the professional-pathway rows added by prompt 027, then shrunk
again 10% -> 5% on 2026-08-12 (prompt 067) to make room for A4 (Show and
Tell Reflection)'s own real weighted row -- see `grading-model.md`'s "How
the final 100% was reached" for the full reconciliation record.
...
`docs/grading-model.md`'s "Attendance & participation" row, 5% of the
...
- `grading_type`: `"points"`.
- `points_possible`: 5 (traces directly to the 5% weight, scaled by
  this project's 100-raw-point-per-100%-of-course-grade convention -- see
  `docs/grading-model.md`'s "Canvas/Savnac submission-location mapping").
- Assignment group: "Attendance & Participation" (`group_weight: 5`).
...
category's point-threshold/attendance-policy language was finalized
2026-08-12 (prompt 067) -- see `docs/syllabus.md`'s "Attendance and
participation" section -- so that blocker is closed; only the
data-collection-mechanism idea above remains a real future follow-on.
