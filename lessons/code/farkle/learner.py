...
What this is NOT: real-world reinforcement learning is usually more
sophisticated. We deliberately use one final turn outcome for every
visited decision because the resulting table is small enough to inspect
and explain in CS1.
"""
...
class ExperienceTable:
    ...
    def _state_key(self, state):
        """Reduce the full state to the small printed CS1 table key."""
    ...
    def as_strategy(self):
        """Wrap the table as a greedy strategy after training."""
    ...
    def table_rows(self):
        """Return readable rows whose displayed preference matches policy."""
        state_keys = []
        ...
        for state_key in state_keys:
            ...
            # This exact comparison is the same one choose_action(...,
            # explore=False) uses. Sparse evidence therefore cannot make the
            # printed table claim a different action than the learner takes.
...
def train_one_turn(rng, table):
    """Play one solo training turn and credit its final outcome."""
    from . import engine
