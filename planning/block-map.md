# COMSC 1033 Fall 2026 — 10–15 Minute Block Map

**The master target list for the content pipeline.** Every in-person 10–15
minute block for the semester, dated against the official academic calendar
(`archive/2026-27-academic-calendar-unofficial.pdf`) and section schedule
(COMSC-1033-1415, MWF 10:00–10:50, Stafford Center 125, Aug 17 – Dec 11).

Holiday facts baked in: **Labor Day Mon Sep 7** (no Monday, week 4) ·
**Fall Break begins Wed Oct 14, 10 PM** (no Friday, week 9) · **Thanksgiving
begins Tue Nov 24, 10 PM** (Monday only, week 15) · **Finals Dec 7–11**.

**Block counts: 15 Monday Moments · 15 Wacky Wednesdays · 14 Fun Fridays =
44 strand blocks**, plus 15 Monday lecture packages.

Per block, the pipeline produces (priority order): **1** digest doc
(LaTeX/Markdown) → **2** Beamer deck (≤15 min, presented via pdfpc/pympress
timer) → **3** podcast → **4** graphic novel chapter → **5** *(pipe dream)*
3–5 min AI-animated video. Checkboxes track the first three; 4–5 ride along
when the pipeline can.

## Lens-1 adjustment (proposed — poke here first)

Universal Week 1 displaced **AI I Lens 1: Define the Problem**. Proposal:
fold Lens 1 into Week 2's Monday Moment — "Define the Problem + Gather
Context" are two halves of the same move (set up the ask). All other lenses
stay in their planned weeks. Alternative: shift every lens down one week and
merge Lens 16 into the final reflection.

## Monday Moments (AI fluency — `ai_fluency/ai_i/`)

| # | Date | Week | Topic | Digest | Deck | Podcast |
|--|---|--|---|--|--|--|
| M01 | Aug 17 | 1 | Getting the most out of this class and this semester (universal Week 1) | ☐ | ☐ | ☐ |
| M02 | Aug 24 | 2 | Lens 1+2: Define the Problem + Gather Context *(proposed merge)* | ☐ | ☐ | ☐ |
| M03 | Aug 31 | 3 | Lens 3: Plan the Work | ☐ | ☐ | ☐ |
| — | ~~Sep 7~~ | 4 | **Labor Day — no class.** Lens 4 (Decompose the Task) folds into Wed Sep 9 intro | | | |
| M04 | Sep 14 | 5 | Lens 5: Select the Right Model | ☐ | ☐ | ☐ |
| M05 | Sep 21 | 6 | Lens 6: Engineer the Prompt | ☐ | ☐ | ☐ |
| M06 | Sep 28 | 7 | Lens 7: Research and Retrieve (RAG) | ☐ | ☐ | ☐ |
| M07 | Oct 5 | 8 | Lens 8: Reason | ☐ | ☐ | ☐ |
| M08 | Oct 12 | 9 | Lens 9: Generate | ☐ | ☐ | ☐ |
| M09 | Oct 19 | 10 | Lens 10: Critique | ☐ | ☐ | ☐ |
| M10 | Oct 26 | 11 | Lens 11: Verify | ☐ | ☐ | ☐ |
| M11 | Nov 2 | 12 | Lens 12: Revise | ☐ | ☐ | ☐ |
| M12 | Nov 9 | 13 | Lens 13: Decide | ☐ | ☐ | ☐ |
| M13 | Nov 16 | 14 | Lens 14: Automate | ☐ | ☐ | ☐ |
| M14 | Nov 23 | 15 | Lens 15: Measure | ☐ | ☐ | ☐ |
| M15 | Nov 30 | 16 | Lens 16: Reflect and Improve | ☐ | ☐ | ☐ |

## Wacky Wednesdays (professional minds — `professional_minds/`)

| # | Date | Week | Book / topic | Weekly question | Digest | Deck | Podcast |
|--|---|--|---|---|--|--|--|
| W01 | Aug 19 | 1 | Getting the most out of your degree (universal Week 1: LinkedIn Learning, degree check/plan, progress report, subs, audits) | — | ☐ | ☐ | ☐ |
| W02 | Aug 26 | 2 | *Make It Stick* | How does the brain improve? | ☐ | ☐ | ☐ |
| W03 | Sep 2 | 3 | *Limitless Mind* | How do we keep growing through challenges? | ☐ | ☐ | ☐ |
| W04 | Sep 9 | 4 | *Critical Thinking* (+ Lens 4 folded intro) | How do we make better decisions? | ☐ | ☐ | ☐ |
| W05 | Sep 16 | 5 | *The Art of Thinking Clearly* | How do we avoid common thinking mistakes? | ☐ | ☐ | ☐ |
| W06 | Sep 23 | 6 | *Statistics Done Wrong* | How do we know something is true? | ☐ | ☐ | ☐ |
| W07 | Sep 30 | 7 | *Understanding by Design* | How do we design meaningful learning? | ☐ | ☐ | ☐ |
| W08 | Oct 7 | 8 | *The Pragmatic Programmer* | What does professional software craftsmanship look like? | ☐ | ☐ | ☐ |
| W09 | Oct 14 | 9 | *Refactoring* (Fall Break begins 10 PM — class meets) | How do systems stay healthy over time? | ☐ | ☐ | ☐ |
| W10 | Oct 21 | 10 | *Software Engineering* | How do teams build quality software? | ☐ | ☐ | ☐ |
| W11 | Oct 28 | 11 | *Software Project Management* | How do projects become reliable products? | ☐ | ☐ | ☐ |
| W12 | Nov 4 | 12 | *Getting Things Done* | How do professionals sustain performance? | ☐ | ☐ | ☐ |
| W13 | Nov 11 | 13 | *97 Things Every Programmer Should Know* | How do professionals work with others? | ☐ | ☐ | ☐ |
| W14 | Nov 18 | 14 | *Docs for Developers* | How do we communicate with humans and AI? | ☐ | ☐ | ☐ |
| — | ~~Nov 25~~ | 15 | **Thanksgiving — no class** | | | | |
| W15 | Dec 2 | 16 | *Generative AI Design Patterns* | What kind of professional do I want to become? | ☐ | ☐ | ☐ |

## Fun Fridays (professional minds — `professional_minds/`)

| # | Date | Week | Book / topic | Weekly question | Digest | Deck | Podcast |
|--|---|--|---|---|--|--|--|
| F01 | Aug 21 | 1 | Getting the most out of your career (universal Week 1: daily 3-things journal; weekly vita paragraphs; monthly resume→CV rotation; internships, resume tips, job search) | — | ☐ | ☐ | ☐ |
| F02 | Aug 28 | 2 | *Mindset* | How does the brain improve? | ☐ | ☐ | ☐ |
| F03 | Sep 4 | 3 | *Resilience Education* | How do we keep growing through challenges? | ☐ | ☐ | ☐ |
| F04 | Sep 11 | 4 | *Thinking, Fast and Slow* | How do we make better decisions? | ☐ | ☐ | ☐ |
| F05 | Sep 18 | 5 | *How Not to Be Wrong* | How do we avoid common thinking mistakes? | ☐ | ☐ | ☐ |
| F06 | Sep 25 | 6 | *Understanding Statistics and Experimental Design* | How do we know something is true? | ☐ | ☐ | ☐ |
| F07 | Oct 2 | 7 | *Rethinking Grading* | How do we design meaningful learning? | ☐ | ☐ | ☐ |
| F08 | Oct 9 | 8 | *Clean Code* | What does professional software craftsmanship look like? | ☐ | ☐ | ☐ |
| — | ~~Oct 16~~ | 9 | **Fall Break — no class.** *Growing Object-Oriented Software* pairing skipped, not made up | | | | |
| F09 | Oct 23 | 10 | *Agile Software Development* | How do teams build quality software? | ☐ | ☐ | ☐ |
| F10 | Oct 30 | 11 | *Growing Object-Oriented Software, Guided by Tests* | How do projects become reliable products? | ☐ | ☐ | ☐ |
| F11 | Nov 6 | 12 | *Joy on Demand* | How do professionals sustain performance? | ☐ | ☐ | ☐ |
| F12 | Nov 13 | 13 | *How to Win Friends and Influence People* | How do professionals work with others? | ☐ | ☐ | ☐ |
| F13 | Nov 20 | 14 | *Prompt Engineering for Generative AI* | How do we communicate with humans and AI? | ☐ | ☐ | ☐ |
| — | ~~Nov 27~~ | 15 | **Thanksgiving — no class** | | | | |
| F14 | Dec 4 | 16 | Semester Reflection | What kind of professional do I want to become? | ☐ | ☐ | ☐ |

**Friday NRP spotlight:** a recurring instructor-driven segment inside
show-and-tell — Jeremy demos something real on the National Research
Platform most weeks ("here's where this road leads"). Weekly dedication,
instructor's showcase; students watch, not operate, in CS1.

## Monday lecture packages (35–40 min main topic)

Each needs: LaTeX section → Beamer deck → "meat" podcast; worked examples
happen live in class.

| # | Date | Week | Technical topic | LaTeX | Deck | Podcast |
|--|---|--|---|--|--|--|
| L01 | Aug 17 | 1 | Foundations: `print`, `input`, execution, environment | ☐ | ☐ | ☐ |
| L02 | Aug 24 | 2 | Variables, expressions, and types | ☐ | ☐ | ☐ |
| L03 | Aug 31 | 3 | Types, conversion, formatting | ☐ | ☐ | ☐ |
| — | — | 4 | **Branching — no Monday (Labor Day).** Lecture content moves to Wed/Fri active time or the podcast carries it | ☐ | ☐ | ☐ |
| L04 | Sep 14 | 5 | Loops and repetition I | ☐ | ☐ | ☐ |
| L05 | Sep 21 | 6 | Loops II; Coding Odyssey planning | ☐ | ☐ | ☐ |
| L06 | Sep 28 | 7 | Functions and decomposition | ☐ | ☐ | ☐ |
| L07 | Oct 5 | 8 | Strings and text processing | ☐ | ☐ | ☐ |
| L08 | Oct 12 | 9 | Collections: lists and dictionaries | ☐ | ☐ | ☐ |
| L09 | Oct 19 | 10 | Collections continued; testing and edge cases | ☐ | ☐ | ☐ |
| L10 | Oct 26 | 11 | Collections applied; reliability | ☐ | ☐ | ☐ |
| L11 | Nov 2 | 12 | Classes, objects, modules I | ☐ | ☐ | ☐ |
| L12 | Nov 9 | 13 | Classes, objects, modules II | ☐ | ☐ | ☐ |
| L13 | Nov 16 | 14 | GitHub, pair programming, AI-aware coding | ☐ | ☐ | ☐ |
| L14 | Nov 23 | 15 | Consolidation; independent project work | ☐ | ☐ | ☐ |
| L15 | Nov 30 | 16 | Coding Odyssey final pass; professional artifacts | ☐ | ☐ | ☐ |

## Production totals

| Artifact | Count |
|---|---:|
| Strand digests | 44 |
| Strand Beamer decks (≤15 min) | 44 |
| Lecture LaTeX sections + decks | 15 + 16 (wk-4 branching ships as materials even without a Monday) |
| Podcasts (44 strand + 16 lecture-meat) | 60 |
| Graphic novel chapters (ride-along) | as pipeline allows |
| Animated 3–5 min videos (pipe dream) | as pipeline allows |

**Grind order recommendation:** Week 1's four packages first (M01, W01, F01,
L01 — teach-ready proof of the whole pipeline), then Weeks 2–4, then batch
the rest via Codex gpt-5.6-luna medium one lesson at a time.

## Poke points (open for Jeremy)

1. Lens-1 merge into M02 — or shift-all-lenses alternative?
2. Week 4 branching: no Monday — does the lecture podcast carry the meat
   with Wed/Fri absorbing the live examples, or does branching compress
   into Week 5's Monday?
3. Week 9 Friday's skipped book (*Growing Object-Oriented Software* appears
   again F10 anyway — planner already handled it; confirm).
4. The 60-podcast total is real; the automated pipeline is load-bearing.
   First pipeline run should be W1's four podcasts.
5. Online section 1414 runs on the same content but has no MWF periods —
   decide its rhythm (async digests + podcasts may *be* the delivery).
