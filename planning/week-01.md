# Week 1 — Success Foundations (Aug 17–21)

## Status
Full week.

## Weekly Focus
Success Foundations — human orientation only. **No programming content of
any kind this week** (Jeremy's pinned decision, 2026-08-12 reconciliation,
`reports/012_pre_savnac_source_reconciliation.md`): no `print`/`input`
instruction, no environment setup framed as a programming lesson, no first
Python program, no paired programming, no Hello World show-and-tell, no
Coding Odyssey work, and no CS1-specific technical gate. Week 1 stays
identical across all of Jeremy's courses, built around three questions:
do I want to keep this class/semester, do I want to keep this degree/path,
and what career/life am I starting to build.

**Consequence: C01 (run/read/change Python) and the technical on-ramp move
to Week 2.** See `planning/week-02.md` and
`planning/coding-odyssey-arc-map.md` — Week 2 is now where `lessons/01-foundations-print-input.md`'s
content is actually taught, alongside variables/expressions/types and the
first Odyssey gate.

> **Week 1 is universal across all of Jeremy's courses** (2026-07-15, see
> `docs/course-ethos.md` / `swosu_cs_curriculum/shared/weekly-rhythm.md`).
> The strand topics below supersede the earlier AI-lens/book pairings for
> this week; those shift to Week 2 onward.

## Week 1 student reset message

The durable cross-course source for the Week 1 semester → degree/education →
career → life message is:

`jeremy-evert/swosu_cs_curriculum/shared/first-week/semester-degree-career-map-announcement.md`

The CS1 Canvas-delivery copy is:

`canvas/announcements/week-01-semester-degree-career-reset.md`

This message is part of the Week 1 curriculum, not merely a Fall 2026 launch
apology. The semester-specific dates and incident language may change, but the
core student task and ownership arc should survive term to term.

**Online-section invariant:** students enrolled in an online section are never
required to attend a face-to-face class meeting or other face-to-face course
interaction. Live classroom or Zoom participation may be offered as optional
enrichment. Any activity described as “in-class” must have an asynchronous
path for online students. The online course must always be completable without
physical presence on campus.

## Monday — Aug 17 — Monday Moments
**Getting the most out of this class and this semester** — best practices for the course itself, and how to get the most from this semester specifically. No technical content.

## Wednesday — Aug 19 — Wacky Wednesday
**Getting the most out of your degree** — LinkedIn Learning, degree check, degree plan, progress report, course substitutions, degree audits.

## Friday — Aug 21 — Fun Friday
**Getting the most out of your career** — daily 3-things journal; weekly two paragraphs into your vita; monthly rotate a resume line into the CV; internships, resume tips, surviving the job search.

## Due this week
Getting-to-know-you quiz (`quizzes/Q1-getting-to-know-you.md`). No coding
submission is due this week.

**Note (2026-07-24):** an earlier pass put the Coding Odyssey genre pick
here, paired with this day's dream-job theme. Moved to Monday of Week 2
instead — Week 1 stays universal across all five courses with no CS1-only
exception, and Week 2 is where the Odyssey's own weekly-gate structure
actually starts. See `planning/week-02.md` and
`planning/coding-odyssey-arc-map.md`.

**Note (2026-08-12):** this week previously also carried `print`/`input`
foundations, environment setup as a programming lesson, paired-programming
first-program work, and a Hello World show-and-tell. All of that was
removed to satisfy the "no programming at all in Week 1" pinned decision;
see `planning/week-02.md`'s Monday block for where it now lives.

**Professional-pathway portfolio — baseline established here (Decision, 2026-07-22, `reports/005_apply_pre_semester_decisions.md`).** Wednesday/Friday's degree-check and career content is where students start or submit the baseline artifacts for `assignments/A6-professional-pathway-artifacts.md`: unofficial transcript/degree audit, advisor/progress check, a preliminary degree plan, a current (even incomplete) resume, current GitHub/LinkedIn, and an initial dream-job/role interest. These don't need to be polished in Week 1 — they get updated in Week 14 and completed/submitted in Week 15. Largely covered by `semester_kickoff_week`'s A01–A09 assignment set; see that repo for the actual handouts/rubrics once CS1's Week 1 sources from it directly (tracked, not yet done as of this decision).
